			...
			for each in display:
				if each['pid'] not in post_list: # If the post has not been displayed yet (in case of multiples of the same post)
					...
					print('Result ' + str(num+1) + '\n\n' + 'postID: ' + str(each['pid']) + '\n' + 
						...
						'Date: ' + str(each['pdate']) + '\n' + 'Poster: ' + str(each['poster']) + '\n' +
						'Number of votes: '+ str(each['vcount']) + '\n' + 
						'Number of answers: ' + ('N/A' if each['acount'] == 0 and each['qpid'] == None
						else str(each['acount'])) + '\n' + 'Body: ' +  str(each['body'])) 
...
def mark_accepted(post_id):
    ...
    c.execute("SELECT theaid FROM questions q WHERE q.pid =:ourPid ",{"ourPid":question} )
    
    rows = c.fetchall() # Only one output is to be expected
    
    if len(rows) != 0 and rows[0][0] != None: # If an accepted answer exists
        acc_answer = rows[0][0]  # The postID of the accepted answer
        # Gets input from the user if they would like to change the accepted answer
        print("An accepted answer already exists it is "+ str(acc_answer) +".\n")
    ...
    if a_exists == '1': # If the user wants to change the accepted answer
            ...
            if rows[0][0] == post_id:
                    print("This post is already the accepted answer of the question. No changes are made.")
            # --------------------------------------------------------------------------------

            else: # Changes the current accepted answer to the answer that the user has chosen
                c.execute("UPDATE questions SET theaid = :ourPid WHERE pid = :ourQuestion;",{"ourPid":post_id,"ourQuestion":question})
...
def give_badge(post_id):
    ...
    if len(row) == 0:
        print("Sorry there are no badges to choose from!")
        return 
